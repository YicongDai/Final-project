...
public class ShopCartBean {
	...
	public void add(String sid) {
		if (hm.containsKey(sid)) {
			...
			int xc = ((Integer) hm.get(sid)).intValue();
			// 把上面获取的xc值存入hm中
		...
		total = total + Double.parseDouble(MongodbBean.getPrice(sid)); // 购买物品后，计算物品总价格
	}
	...
	public void setCount(int c, String sid) {
		...
		total = total + (c - yc) * Double.parseDouble(MongodbBean.getPrice(sid));
	...
	public void deleteFromCart(String sid) {
		int yc = ((Integer) hm.get(sid)).intValue();
		total = total - yc *Double.parseDouble( MongodbBean.getPrice(sid));
		hm.remove(sid);
	}
	...
	public String[][] getCart() {
		...
		for (int i = 0; i < size; i++) {
			...
			sa[1] = MongodbBean.getName(sid); // 获取购买货物名称
			sa[2] = MongodbBean.getPrice(sid); // 获取购买货物价格
			sa[3] = MongodbBean.getSeller(sid); // 获取购买货物主人
			sa[4] = ((Integer) hm.get(sid)).toString(); // 获取购买货物数量
